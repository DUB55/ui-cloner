...
export default function Layer({
  ...
  return (
    <Rnd
      ...
      bounds="parent"
      onDragStop={(_e, d) => {
        onChange && onChange({ bbox: { ...bbox, x: Math.round(d.x), y: Math.round(d.y) } });
      }}
      onResizeStop={(_e, _dir, ref, _delta, pos) => {
        onChange && onChange({ bbox: { x: Math.round(pos.x), y: Math.round(pos.y), width: Math.round(ref.offsetWidth), height: Math.round(ref.offsetHeight) } });
      }}
      onClick={() => onSelect && onSelect()}
      style={{
        border: isSelected ? '2px solid #1e88e5' : '1px dashed rgba(0,0,0,0.15)',
